    ...
    if (!user) {
      try {
        ...
        await db.upsertUser({
          ...
          firstName: userInfo.name?.trim().split(/\s+/)[0] || "Sistem",
          lastName: userInfo.name?.trim().split(/\s+/).slice(1).join(" ") || "Kullanıcısı",
...
function buildCronUser(
  ...
  return {
    ...
    openId: userInfo.openId,
    firstName: "Sistem",
    lastName: "Zamanlayıcı",
    name: userInfo.name || "Manus Scheduled Task",
    email: "cron@local.invalid",
    passwordHash: "!cron-only!",
