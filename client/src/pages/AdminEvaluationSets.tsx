...
export default function AdminEvaluationSets() {
  ...
  const [criterionDraft, setCriterionDraft] = useState<CriterionDraft>(buildEmptyCriterionDraft());
  ...
  const totalCriterionPoints = useMemo(
    () => criterionDraft.controlPoints.filter(point => point.trim().length > 0).length,
  ...
  const openCriterionEdit = (criterion: { id: number; name: string; description?: string | null; controlPoints?: string[] }) => {
    setEditingCriterionId(criterion.id);
    setCriterionDraft({
      name: criterion.name,
      description: criterion.description ?? "",
      controlPoints: Array.isArray(criterion.controlPoints) && criterion.controlPoints.length > 0
    ...
    <AppShell role="ADMIN">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-semibold tracking-tight text-[#0b385d]">Değerlendirme Setleri</h1>
          <p className="mt-2 text-sm text-slate-500">Farklı değerlendirme setlerini tanımlayın, rubrik ölçeğini ve ağırlıkları yönetin.</p>
        </div>
        <Button className="gap-2 bg-[#0b385d] hover:bg-[#082d4c]" onClick={openCreate}>
            ...
            <CardContent className="space-y-4 p-5">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  {set.description && <p className="mt-1 text-sm text-slate-500">{set.description}</p>}
                </div>
                  <div className="flex items-center gap-2">
                    <span className="rounded-full bg-amber-50 px-2 py-1 text-[11px] font-medium text-amber-700">Baraj: {Number(set.passingScore ?? 70)}%</span>
                    <span className="rounded-full bg-teal-50 px-2 py-1 text-[11px] font-medium text-teal-700">{set.criteria.reduce((sum, item) => sum + Number(item.weight ?? 0), 0)} / 100</span>
                  </div>
              </div>
              <div className="space-y-2">
                <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Rubrik</p>
                <div className="flex flex-wrap gap-2">
                  {Object.entries(set.rubricScale ?? {}).map(([score, label]) => (
                    <span key={score} className="rounded-full bg-slate-100 px-2 py-1 text-[11px] text-slate-700">{score}: {label}</span>
                  ))}
                </div>
              </div>
              <div className="space-y-2">
                <p className="text-xs font-medium uppercase tracking-wide text-slate-500">Kriterler</p>
                <div className="flex flex-wrap gap-2">
                  {set.criteria.map(item => (
                    <span key={item.criterionId} className="rounded-full bg-amber-50 px-2 py-1 text-[11px] text-amber-700">{item.name} ({item.weight})</span>
                  ))}
                </div>
              </div>
              <div className="flex justify-end gap-2">
                <Button variant="outline" size="sm" onClick={() => openEdit(set.id)}>Düzenle</Button>
                <Button variant="outline" size="sm" className="gap-2 text-rose-600 hover:text-rose-700" onClick={() => remove.mutate({ id: set.id })} disabled={!set.isActive}>
        ...
        <div className="fixed inset-0 z-[80] grid place-items-center bg-slate-950/40 p-4">
          <div className="w-full max-w-2xl rounded-2xl bg-white p-6 shadow-2xl">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-sm font-medium text-teal-700">{editingCriterionId ? "Kriter düzenle" : "Yeni kriter"}</p>
                <h2 className="mt-1 text-xl font-semibold text-[#0b385d]">{editingCriterionId ? "Kriter bilgilerini güncelle" : "Yeni kriter ekle"}</h2>
              </div>
              <Button variant="outline" onClick={() => { setCriterionEditorOpen(false); setEditingCriterionId(null); setCriterionDraft(buildEmptyCriterionDraft()); }}>
                Kapat
              </Button>
            </div>
            ...
            <div className="mt-5 space-y-4">
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={event => setCriterionDraft(current => ({ ...current, name: event.target.value }))}
              ...
              <div className="space-y-2">
                ...
                <Textarea
                  ...
                  onChange={event => setCriterionDraft(current => ({ ...current, description: event.target.value }))}
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <span className="text-xs text-slate-500">{totalCriterionPoints} adet</span>
                </div>
                ...
                {criterionDraft.controlPoints.map((point, index) => (
                  <div key={index} className="flex items-center gap-2">
                    <Input
                      value={point}
                      onChange={event => updateCriterionPoint(index, event.target.value)}
                ...
                <Button variant="outline" className="gap-2" onClick={addCriterionPoint}>
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              <Button className="bg-[#0b385d] hover:bg-[#082d4c]" onClick={submitCriterion} disabled={createCriterion.isPending || updateCriterion.isPending}>
                {createCriterion.isPending || updateCriterion.isPending ? "Kaydediliyor..." : "Kaydet"}
              </Button>
            </div>
          </div>
        </div>
      ...
      {open && (
        <div className="fixed inset-0 z-[70] grid place-items-center bg-slate-950/40 p-4">
          <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
            <div className="flex items-center justify-between gap-3">
              <div>
                <p className="text-sm font-medium text-teal-700">{editingId ? "Set düzenle" : "Yeni set"}</p>
                <h2 className="mt-1 text-xl font-semibold text-[#0b385d]">{editingId ? "Değerlendirme seti" : "Yeni değerlendirme seti"}</h2>
              </div>
            ...
            <div className="mt-6 grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
              <div className="space-y-4">
                <div className="space-y-2">
                  <Label>Set adı</Label>
                  <Input value={form.name} onChange={event => setForm(current => ({ ...current, name: event.target.value }))} placeholder="Ör: E-eğitim Değerlendirme" />
                </div>
                <div className="space-y-2">
                  <Label>Açıklama</Label>
                  <Textarea value={form.description} onChange={event => setForm(current => ({ ...current, description: event.target.value }))} rows={3} placeholder="Setin amacını açıklayın..." />
                </div>
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    placeholder="70"
                  />
                  <p className="text-xs text-slate-500">Değerlendirme yüzdesi bu değere eşit veya yüksekse sonuç başarılı sayılır.</p>
                </div>
                <div className="space-y-3">
                  <Label>Rubrik ifadeleri</Label>
                  {Object.entries(form.rubricScale).map(([score, label]) => (
                    <div key={score} className="flex items-center gap-2">
                      <span className="grid h-8 w-10 place-items-center rounded-lg bg-slate-100 text-sm font-semibold text-slate-700">{score}</span>
                      <Input value={label} onChange={event => setForm(current => ({ ...current, rubricScale: { ...current.rubricScale, [score]: event.target.value } }))} />
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="space-y-4">
                <div className="rounded-xl border border-slate-200 bg-slate-50 p-4">
                  <div className="flex items-center justify-between gap-3">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-medium text-slate-500">Toplam {totalWeight.toFixed(1)} / 100</span>
                      <Button type="button" variant="outline" size="sm" className="gap-1" onClick={openCriterionCreate}>
                      ...
                      <div key={criterion.id} className="rounded-lg border border-slate-200 bg-white p-2">
                        <div className="flex items-start justify-between gap-3">
                          <label className="flex flex-1 items-start gap-2 text-sm text-slate-700">
                            <input type="checkbox" checked={form.selectedCriterionIds.includes(criterion.id)} onChange={() => toggleCriterion(criterion.id)} />
                            <span>{criterion.name}</span>
                          </label>
                          <div className="flex items-center gap-1">
                            <Button type="button" variant="ghost" size="icon" className="h-7 w-7" onClick={() => openCriterionEdit(criterion)}>
                              <Pencil className="h-3.5 w-3.5" />
                            </Button>
                            <Button type="button" variant="ghost" size="icon" className="h-7 w-7 text-rose-600 hover:text-rose-700" onClick={() => removeCriterion.mutate({ id: criterion.id })}>
                              <Trash2 className="h-3.5 w-3.5" />
                            </Button>
                          </div>
                        </div>
                        <div className="mt-2 flex items-center justify-between gap-3">
                          <span className="text-[11px] text-slate-500">{criterion.controlPoints.length} kontrol</span>
                          {form.selectedCriterionIds.includes(criterion.id) && (
                            <div className="flex items-center gap-2">
                              <Input
                                ...
                                onChange={event => updateWeight(criterion.id, event.target.value)}
                                className="w-24"
                              />
                              <span className="text-xs text-slate-500">ağırlık</span>
                            </div>
                          )}
                        </div>
                      </div>
            ...
            <div className="mt-6 flex justify-end gap-2">
              ...
              <Button className="gap-2 bg-[#0b385d] hover:bg-[#082d4c]" onClick={submit} disabled={create.isPending || update.isPending}>
                <Save className="h-4 w-4" /> {create.isPending || update.isPending ? "Kaydediliyor..." : "Kaydet"}
              </Button>
            </div>
          </div>
        </div>
